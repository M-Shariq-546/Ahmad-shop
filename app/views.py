...
def get_subcategories(request):
    ...
    subcategories = Category.objects.filter(parent_id=category_id).values(
        'id', 'name', 'slug')

    # Add `has_subcategories` flag
    subcategories_list = list(subcategories)
    for subcategory in subcategories_list:
        subcategory['has_subcategories'] = Category.objects.filter(
            parent_id=subcategory['id']).exists()
...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['newest_products'] = Product.objects.filter(
            in_stock=True).order_by('-id')[:16].only('name', 'slug', 'price',
                                                     'discount_price',
                                                     'images', 'is_top_seller')
    ...
    def get_queryset(self):
        ...
        for key, values in filters.lists():
            if key in ['min_price', 'max_price', 'sort',
                       'page']:  # Excluding 'page' to avoid conflicts
                continue
...
def search(request):
    ...
    if request.method == 'POST':
        ...
        q = Product.objects.filter(
            Q(name__icontains=q_user) | Q(description__icontains=q_user))
